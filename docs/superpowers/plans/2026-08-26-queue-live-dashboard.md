...
## ✅ 完成（commit `61fd242` on `h16-recovery`）

### 修复内容

最终总结阶段有 4 类 LLM 调用，LaneView 之前只显示分析阶段的残留（截图里 `ch1669-ch1648`）。本 PR 在 **frontend 翻译 `summary_progress` 事件**为 LaneView 兼容的 `activeBlocks`/`finishedBlocks` 操作，**零后端改动**。

抽出独立 helper `frontend/src/utils/summaryLanes.ts`（可独立 vitest）：

**合成 id 范围**（避开分析阶段 1-10000 的章节 id）：

- batch 阶段  : 900001-900999（并发卷分析）
- recheck 阶段: 910001-910999（伏笔复检）
- style 阶段  : 920001（单次 LLM）
- report 阶段 : 930001（单次 LLM）

**事件处理**：

- `phase=batch/recheck/style/report` 阶段开始 → 上一阶段残留合成块（id ≥ 900000）移到 finished 带 ✓，分析阶段原 id 1-10000 不动；再合成新阶段块
- `batch_done / batch_failed` → 对应合成块从 active 移到 finished（failed 时 `ok=false`）
- `complete` → 残留的 style/report 单次块（无 batch_done 事件）兜底标 done

### 🧪 验证

- **62/62 通过**（52 → 62，+10 新用例覆盖 phase/batch_done/failed/sweep/complete/防御分支）
- vite 5.00s / 0 错（本次 dist 用 vite 单独产出，含本次 LaneView 兼容改动）
- 新 hash `index-Qh3PCX3B.js` / `index-CbX0oYbl.css` 已拷到 share

### ⚠️ 一个 build 提示

`vue-tsc` 报错：`SettingsPage.vue` 引用 `ProbeThinkingResult.default_detection_breakdown`，但 `client.ts` 的 `ProbeThinkingResult` 类型缺这个字段。**这是其他 Agent 未提交代码的 bug**（他们加 SettingsPage 用法但没补类型）。本次保留他们 3 个文件 ` M` unstaged，让他们先修类型。`vite build` 单独跑没问题，dist 是新的。

### 🎬 你这边

重启桌面：

- 进入最终总结阶段，LaneView 会自动从分析残留 → 清空 → 合成 12 个"卷X/12"块
- 阶段切换时（batch → style → report）残留合成块自动带 ✓
- batch_done 时合成块从 active 移到 finished
- 顶部 hint 横幅 + LaneView 两边同时显示总结阶段进度

**Scope 守住**：3 个其他 Agent 的文件（`test_anthropic_provider.py` / `client.ts` / `SettingsPage.vue`）仍是 ` M` unstaged。
