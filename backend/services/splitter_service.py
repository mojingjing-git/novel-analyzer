...
def infer_book_name(file_path: Path) -> str:
    ...
    name = file_path.stem
    # 主路径：截取《...》内容
    m = re.search(r"[《<]([^》>]{1,80})[》>]", name)
    if m:
        return m.group(0)  # 保留《》本身
    # 回退：剥离已知版本后缀（处理无《》的书名；同时覆盖带/不带括号变体）
    for suffix in [
        # 带括号（最常见）
        "（校对版全本）", "(校对版全本)", "（全本）", "(全本)",
        "（校对版）", "(校对版)", "（完整版）", "(完整版)",
