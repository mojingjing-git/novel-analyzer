...
useProgressSocket((msg: ProgressMessage) => {
  ...
  if (msg.type === 'log') {
    ...
    store.add(text, level as 'info' | 'warn' | 'error' | 'state', category)
  } else if (msg.type === 'state_change') {
    const state = msg.payload.state as string
    const detail = (msg.payload.detail as string) ?? ''
    store.add(`状态: ${state} - ${detail}`, 'state', 'system')
    ...
    if (ok) {
      store.add(`第${ch}章分析完成 (${elapsed.toFixed(1)}s)`, 'info', 'analysis')
    } else {
      store.add(`第${ch}章分析失败`, 'error', 'analysis')
    }
  }
})
</script>
